...
import { Star, MessageSquareQuote, Calendar, Clock, X, ArrowLeft, BookOpen } from 'lucide-react';
...
export default function ConseilsContent() {
  ...
  // Sync state with url search parameter ?id=
  useEffect(() => {
    const id = searchParams.get('id');
    if (id && ARTICLES.some(a => a.id === id)) {
      setSelectedArticleId(id);
      // Scroll to active blog container if selected
  ...
  return (
    <div className="w-full">
      ...
      <section className="bg-[#FFFAFA] py-12 md:py-20 border-b border-[#E8D8CC]/50">
        <div className="max-w-[1200px] mx-auto px-6 md:px-8 text-center">
          <div className="inline-flex items-center justify-center gap-2 border border-[#C9AA95]/40 bg-[#F8F0EB] px-3 py-1 rounded-none mb-6">
            <BookOpen size={14} strokeWidth={1.5} className="text-[#7A5C46]" />
          ...
          <h1 className="font-display text-[34px] md:text-[48px] font-medium text-[#573119] mb-4">
            Conseils <em className="font-display italic text-[#7A5C46] font-normal">d&apos;Apothicaire</em>
          </h1>

          <p className="font-body text-[15px] font-light text-[#7A5C46] max-w-xl mx-auto leading-relaxed">
      ...
      <section id="blog-section" className="bg-[#F8F0EB] py-16 md:py-24 border-b border-[#E8D8CC]/50 scroll-mt-24">
        <div className="max-w-[1200px] mx-auto px-6 md:px-8">
          ...
          {selectedArticle ? (
            <div className="max-w-3xl mx-auto bg-[#FFFAFA] rounded-none border border-[#E8D8CC] shadow-none p-6 md:p-10 animate-fade-in mb-12">
              <button 
                onClick={handleCloseArticle}
                className="inline-flex items-center gap-2 text-[12px] font-body font-semibold uppercase tracking-wider text-[#573119] hover:text-[#3E2110] mb-8 cursor-pointer"
              >
                <ArrowLeft size={16} strokeWidth={1.5} />
                Retour aux dossiers
              </button>

              <div className="flex items-center gap-3 font-body text-[11px] text-[#B09880] mb-4">
                <span className="bg-[#F8F0EB] text-[#7A5C46] px-2 py-0.5 rounded-none font-medium">
              ...
              <h2 className="font-display text-[26px] md:text-[36px] font-medium text-[#2C1A0E] leading-snug mb-6">
                {selectedArticle.title}
              </h2>

              <div className="aspect-[16/9] w-full rounded-none overflow-hidden border border-[#E8D8CC] mb-8">
              ...
              <div className="font-body text-[15px] font-light text-[#7A5C46] leading-[1.8] space-y-6">
                {selectedArticle.content.split('\n\n').map((paragraph, index) => (
                  <p key={index}>{paragraph}</p>
                ))}
              </div>

              <div className="mt-10 pt-6 border-t border-[#E8D8CC] flex justify-between items-center">
                <p className="font-display italic text-[14px] text-[#7A5C46]">Rédigé par la Maison Louise XIV</p>
                <button 
                  onClick={handleCloseArticle}
      ...
      <section id="faq-section" className="bg-[#FFFAFA] py-16 md:py-24 border-b border-[#E8D8CC]/50 scroll-mt-24">
        <div className="max-w-[1200px] mx-auto px-6 md:px-8">
          ...
          <div className="text-center max-w-xl mx-auto mb-16">
            ...
            <p className="font-body text-[14px] text-[#7A5C46] font-light mt-2">
      ...
      <section id="reviews-section" className="bg-[#F8F0EB] py-16 md:py-24 border-b border-[#E8D8CC]/50 scroll-mt-24">
        <div className="max-w-[1200px] mx-auto px-6 md:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {CUSTOMER_REVIEWS.map((review) => (
              <div 
                key={review.id} 
                className="bg-[#FFFAFA] p-8 rounded-none border border-[#E8D8CC] shadow-none relative"
              >
                {/* Quote Icon */}
                <div className="absolute top-6 right-8 text-[#E8D8CC] opacity-60">
                ...
                {/* Content */}
                <p className="font-body text-[13.5px] font-light text-[#7A5C46] leading-relaxed mb-6 italic">
                  &quot;{review.text}&quot;
                </p>

                {/* Reviewer info */}
                <div className="pt-4 border-t border-[#E8D8CC]/50">
                  <div className="flex items-center justify-between">
                    <h4 className="font-display text-[15px] font-semibold text-[#2C1A0E]">
                      {review.name}
                    </h4>
                    <span className="font-body text-[10px] text-[#B09880]">
                      {review.date}
                    </span>
                  </div>
                  
                  {/* Hair Type Badge */}
                  <div className="inline-block mt-2 bg-[#EAF0E6] text-[#6B8F5E] px-2 py-0.5 rounded-none">
          ...
          <div className="text-center mt-12 max-w-sm mx-auto p-4 rounded-none border border-[#E8D8CC] bg-[#FFFAFA]/50">
            <p className="font-body text-[13px] text-[#7A5C46]">
              Note moyenne globale de la Maison : <span className="font-bold text-[#2C1A0E]">4.9/5</span> basée sur plus de 1200 avis vérifiés.
            </p>
          </div>
